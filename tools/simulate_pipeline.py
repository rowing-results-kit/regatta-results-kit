...
使い方:
  python3 tools/simulate_pipeline.py                         # site/data/results/ に出力
...
必要な環境変数（--push 使用時）:
  ...
  GITHUB_REPO   対象リポジトリ（例: your-org/your-repo）
"""
...
from common import C, log_info, log_ok, log_warn, log_error, log_debug, log_section, log_title, ms_to_formatted, read_csv_text, CsvEncodingError
...
def build_race_json(
    ...
    for point in measurement_points:
        ...
        for rec in point_records[point]:
            ...
            # フィニッシュポイントの tie_group/photo_flag/note を「無条件で」採用する。
            # ※ GAS本番（gas/Code.gs buildRaceJson）と同じ挙動:
            #     laneMap[row.lane].tie_group = row.tie_group;  ← 空文字でも上書き
            #   条件付き上書き（if rec["tie_group"]:）にすると、500m時点の同着グループが
            #   フィニッシュまで残り、実際は差がついた2艇を同着として公開してしまう
            if point == finish_point:
                lane_times[lane]["_tie_group"]  = rec["tie_group"]
                lane_times[lane]["_photo_flag"] = rec["photo_flag"]
                lane_times[lane]["_note"]       = rec["note"]
    ...
    # ※ GAS本番（gas/Code.gs buildRaceJson）と同じ扱い:
    #   フィニッシュタイムのないレーンは除外せず status:'dnf' で結果に残す
    results_raw = []
    dnf_raw     = []
    # レーン昇順で走査する。
    # ※ GAS本番は laneMap のキーが整数のため Object.values() が「レーン昇順」で返る。
    #   Python の dict はCSVの行順のままなので、同着時の並び順が本番と食い違う。
    #   以降の finish_ms ソートは安定ソートなので、ここを揃えると同着の並びも一致する。
    for lane in sorted(lane_times):
        ...
        if finish_point not in times:
            log_warn(f"  レーン {lane}: フィニッシュタイム ({finish_point}) なし → DNF として記録")
            dnf_raw.append({
                "lane":  lane,
                "times": point_times,
                "note":  times["_note"],
    ...
    # 途中棄権（DNF）レーン: 順位なし・完走艇の後ろに並べる
    # ※ GAS本番と同じく tie_group/photo_flag はDNF艇では保持しない
    dnf_raw.sort(key=lambda r: r["lane"])
    for r in dnf_raw:
        results.append({
            "lane":       r["lane"],
            "rank":       None,
    ...
    for race_no in sorted(race_files):
        ...
        for point in measurement_points:
            ...
            try:
                ...
                # 1レースの文字コード不正で全体を止めない（他のレースは処理を続ける）
                log_error(f"  {point}: {e}")
                parse_ok = False
                error_count += 1
                break
        ...
        # 結果プレビュー
        dnf_count = sum(1 for r in race_json["results"] if r.get("status") == "dnf")
        log_info(
            f"  生成結果: {len(race_json['results'])} 艇"
            + (f"（うちDNF {dnf_count} 艇）" if dnf_count else "")
        )
        for r in race_json["results"]:
            finish_fmt = (r.get("finish") or {}).get("formatted", "---")
            tie_str    = f" [{r['tie_group']}]" if r["tie_group"] else ""
            rank_str   = f"{r['rank']:2d}位" if r["rank"] is not None else "DNF"
            print(f"    {C.GRAY}  {rank_str}  レーン{r['lane']}  {finish_fmt}  {r['split']}{tie_str}{C.RESET}")
        ...
        if do_push:
            ...
            try:
                ...
                error_count += 1
                continue
            remote_path = f"{rel.as_posix()}/{filename}"
            try:
                github_push_file(
                    token         = github_token,
                    repo          = github_repo,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        default="site/data/results/",
        metavar="DIR",
        help="race_XXX.json の出力先ディレクトリ（デフォルト: site/data/results/）",
    )
    parser.add_argument(
        "--points",
        default="500m,1000m",
