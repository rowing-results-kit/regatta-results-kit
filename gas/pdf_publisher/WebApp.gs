/**
 * ============================================================
 *  マスターズPDF生成 — 帳票生成ウェブアプリ (WebApp.gs)
 *
 *  目的:
 *    結果ブックレット・結果一覧PDF・レース前準備資料の生成を、
 *    GAS エディタを開かずにブラウザのボタンから実行できるようにする。
...
/**
 * 帳票生成アクションを実行する。
 * @param {string} action - webapp.html のボタンに対応するアクション名
 * @param {string} dateStr - 日付指定アクションのみ使用（'YYYY/MM/DD'）
...
function webappRunAction(action, dateStr) {
  try {
    ...
    switch (action) {
      case 'preRaceAll':
        generatePreRaceBooklet();
        return { ok: true, message: '全日程のレース前準備資料を作成しました。Drive の出力フォルダを確認してください。' };

      case 'preRaceDate':
        if (!dateStr) return { ok: false, message: '日付を選んでください。' };
        generatePreRaceBookletForDate(dateStr);
        return { ok: true, message: dateStr + ' のレース前準備資料を作成しました。Drive の出力フォルダを確認してください。' };

      case 'resultsBookletAll':
        generateAllResultsBooklet();
        return { ok: true, message: '全日程の結果ブックレットを作成しました。Drive の出力フォルダを確認してください。' };

      case 'resultsBookletDate':
        if (!dateStr) return { ok: false, message: '日付を選んでください。' };
        generateResultsBookletForDate(dateStr);
        return { ok: true, message: dateStr + ' の結果ブックレットを作成しました。Drive の出力フォルダを確認してください。' };

      case 'resultsListAll':
        generateResultsListPdf();
        return { ok: true, message: '全日程の結果一覧PDFを作成しました。Drive の出力フォルダを確認してください。' };

      case 'resultsListDate':
        if (!dateStr) return { ok: false, message: '日付を選んでください。' };
        generateResultsListPdfForDate(dateStr);
        return { ok: true, message: dateStr + ' の結果一覧PDFを作成しました。Drive の出力フォルダを確認してください。' };
