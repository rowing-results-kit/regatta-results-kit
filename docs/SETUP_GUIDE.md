...
| ジョブが赤バツ（❌） | 入力値の形式エラー | ジョブログを確認 → 日付は YYYY-MM-DD 形式か、カラーは `#` から始まる 7 文字かを確認してやり直す |
...
| 結果が反映されない（5分以上） | GAS トリガー / CSV のフォルダ / ファイル名 | ポータル「状態」タブでトリガー稼働状況を確認 → `onTrigger()` を手動実行 → ファイル名の形式を確認（`R001_500m.csv` 形式が必須） |
