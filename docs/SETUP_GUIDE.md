...
### 自動監視（GAS が止まったら気づく仕組み）

GAS は正常に動いている間、`site/data/master.json` の `last_trigger_at` を定期的に更新し続けます（ハートビート）。GitHub Actions の **GAS heartbeat watchdog**（`.github/workflows/heartbeat-watchdog.yml`）が 10 分ごとにこの時刻を見張り、更新が途絶えていたら GitHub の Issue で通知します。**この監視は設定不要で、大会が近づくと自動で有効になります。**

仕組みは次のとおりです。watchdog は毎回まず `site/data/master.json` の大会日程（`tournament.dates`）を読み、**「大会初日の前日 〜 最終日の翌日」の期間内かどうか**を判定します。期間外なら何もせず終了するため、大会がない時期に無駄な通知や実行が発生しません。期間内に入ると自動で監視を開始し、ハートビートが **大会当日は 15 分**・**前日と翌日は 30 分** を超えて途絶えると Issue を作成します（同じ Issue が開いている間はコメントを追記するだけで、Issue が乱立しません）。つまり **大会日程を登録しておけば、大会のたびに監視を手動で ON/OFF する必要はありません**。日程が未登録の場合は監視はスキップされます。

動作を今すぐ試したいときは、GitHub の Actions タブ →「GAS heartbeat watchdog」→「Run workflow」から手動実行できます（`dry_run` を `true` にすると Issue を作らずに判定結果だけ確認できます）。
