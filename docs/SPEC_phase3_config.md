...
```jsonc
{
  ...
  "default_course": {                    // 新設・必須
    "length_m": 1000,                    // 500 / 1000 / 1500 / 2000 の4値のみ有効（自由入力禁止）
    "measurement_points": [500, 1000]    // 昇順。最終要素 = ゴール（= length_m と一致必須）
                                         // 各要素も 500 / 1000 / 1500 / 2000 のいずれか
  },
...
推奨命名（新規大会向けドキュメント記載用）: `YYYYMMDD_HHMMSS_R{race_no}_{point}m.csv` — `{point}` は `(500|1000|1500|2000)m` のいずれか（4値のみ正）。
