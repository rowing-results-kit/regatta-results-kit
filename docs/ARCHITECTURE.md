...
入力 → 処理 → 出力。GUI版は `docs/system-map.html`。凡例: 🟥PDF / 🟦Web反映 / 🟨Excel・文書 / ⚙️自動 / 👤手動。

```
入力                  処理                              出力
────────────────────────────────────────────────────────────────────────
エントリーCSV    ┐  ⚙️ Code.gs onTrigger(2分毎)      🏁試合前に書き出せるもの
スケジュールCSV  ┼─▶  CSV→race_NNN.json→Push       ・🟥判定員帳票  (judge generateAllJudgeForms 👤)
計測CSV(後着)    ┘  ⚙️👤 pdf_publisher                ・🟥準備資料冊子(generatePreRaceBooklet 👤)
config/theme     ─▶  judge_form_publisher 👤          ・🟥🟨競漕記録(空欄)(generate_race_pdf/_xlsx 👤)
                    tools/ ローカル(技術者) 👤          ・🟨スタッフHTML / 🟨手順書 (scaffold.py 👤)
                    AdminPortal ポータル 👤             ・🟦スケジュール/出漕表示 (サイト ⚙️)
                                                      ・🟦ハブ掲載(association.json👤→反映)
                                                    ───────────────────────────────
                                                      🏆試合結果を元に書き出すもの
                                                      ・🟦順位/タイム表示(onTrigger ⚙️2分毎)
                                                      ・🟥個別レース結果(processPendingPDFs ⚙️)
                                                      ・🟥結果ブックレット(generateAllResultsBooklet 👤)
                                                      ・🟥結果一覧表(generateResultsListPdf 👤)
                                                      ・🟥🟨競漕記録(タイム入り)(generate_race_pdf/_xlsx 👤)
                                                      ・🟦ハブのステータス更新(⚙️/一部👤)
```

注記① 進行計算（次ラウンド自動組分け）は**次フェーズ**。現状ポータルはモデル**選択**(template_id 保存)のみ。
注記② 日常運用で人が触るのは **Google Drive と 管理者ポータル** のみ。GAS・サイト反映は自動、tools/ は技術者の裏方。
