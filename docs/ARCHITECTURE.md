...
| 結果格納 | スクリプトプロパティ `PDF_OUTPUT_FOLDER_ID` で管理 |
| 印刷済・掲出済（手動移動） | 任意（大会ごとに設定） |
| 削除アーカイブ（GAS 自動移動） | スクリプトプロパティ `PDF_ARCHIVE_FOLDER_ID` で管理 |
| 雛形 Sheets 置場 | 任意（大会ごとに設定） |
| 準備資料 PDF | スクリプトプロパティ `OUTPUT_FOLDER_ID` で管理 |
| 研修テスト | 任意（大会ごとに設定） |
