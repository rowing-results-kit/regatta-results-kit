...
#### 5.1.1 公式着順と計測タイムの優先順位

`finish_rank` と `time_ms` は食い違い得る。写真判定・審判裁定・記録訂正はいずれも `finish_rank` に載る一方、`time_ms` は計測システムが捉えた値のまま残るためである。どちらを優先するかは**比較の対象が同一レースかどうか**で決まる。§5.1 の2つの解決関数がそれぞれ別の比較関数を使うのはこのためである。

| 比較の場面 | 優先順位 | 根拠 |
|---|---|---|
| **同一レース内**の着順（`N.M.H` の解決、予選各組の順位付け、プレリミナリー） | status → **finish_rank** → time → input_order | §5.1 `resolveRaceRank` が `compareByFinishRank` で並べる。公式着順がクルーに与えられた結果であり、進出可否を決める |
| **レース跨ぎ**のタイム拾い上げ（`N.HT`、同一ランク保持者どうしの序列） | status → **time** → input_order | §5.1 `resolveHeatTimeRank` と §6 `takeTimeAdvancers` が `compareByTimeWithTie` で並べる |

> ⚠️ レース跨ぎの比較に `finish_rank` を混ぜてはならない。組が違えば着順は比較不能で、ある組の2着が他組の1着より速いことは日常的に起こる。同一ランク保持者どうし（各組1着など）も `finish_rank` が同値なので、序列はタイムでしか決まらない。両者を1つの比較関数にまとめるとタイム拾い上げが壊れる。
...
#### 5.2.1 短編成のレーン配置（欠員がある場合）

テンプレートの `lane_assignment` は満員（`entries_max`）を前提に書かれており、その `bn` は本節の推奨レーン順（6レーンなら `[3, 4, 2, 5, 1, 6]`）でランク順に並んでいる。すなわち最上位ランクが中央のレーン3、次が4、最下位ランクが外側という構成である。

実際の出場艇がレーン数に満たない場合、テンプレートの `bn` をそのまま使ってはならない。片岸にクルーが偏る、あるいはコース中央のレーンが空くという物理的に不適切な発艇表になるためである。欠員が生じる原因は次の2つがあるが、**どちらの場合も同じ規則で配置する**。

| 原因 | 例 |
|---|---|
| エントリー数がパターンの上限に満たない | 22艇で `19.HT` / `20.HT` が存在しない |
| DNS/DNF/DQ により順位が空く | §6 により対象外となった艇の枠 |

規則: **生き残った艇は相対的なランク順を保ったまま、推奨レーン順の先頭 N 枠を占める**。

```typescript
function seatCentreOut(
  rules: LaneAssignment[],
  survivors: Set<SourceIdentifier>,
  lanes: number            // 会場のレーン数（ProgressionTemplate.lanes）。
): Array<{ bn: number; source: SourceIdentifier }> {
  const laneOrder = DEFAULT_LANE_ORDER[lanes];
  if (!laneOrder) {
    throw new ProgressionError("LANE_ORDER_NOT_DEFINED", { lanes });
  }
  // テンプレートの bn は推奨レーン順に並んでいるので、その位置がランクを表す
  const byRank = rules
    .slice()
    .sort((a, b) => laneOrder.indexOf(a.bn) - laneOrder.indexOf(b.bn))
    .filter(rule => survivors.has(rule.source));
  return byRank.map((rule, index) => ({ bn: laneOrder[index], source: rule.source }));
}
```

`lanes` は必ず `ProgressionTemplate.lanes` を渡す。レース自身の `bn` の最大値から会場を推測してはならない。会場の最外レーンまで使わないレース（6レーン会場で5艇分だけ定義されたレースなど）を、より小さい会場と取り違えて別の順を引いてしまう。

`bn` が中央外向き順として読めないテンプレートは着席させず、次のとおり弾く。推測して埋めると「レーン番号の無い艇」や「同一レーンに2艇」といった物理的に不可能な発艇表を、正しいものとして出力してしまうためである。

| 条件 | エラー |
|---|---|
| `lanes` が `DEFAULT_LANE_ORDER` に無い | `LANE_ORDER_NOT_DEFINED` |
| `bn` が会場のレーンに存在しない（0・負・レーン数超過） | `LANE_OUT_OF_RANGE` |
| 同じ `bn` が複数の `source` に割り当てられている | `LANE_DUPLICATED` |

この2つを通れば「相異なる実在レーンが艇数分ある」ことが保証されるため、艇数がレーン数を超える形（§4.3 の `LANE_CAPACITY_EXCEEDED`）は `LANE_DUPLICATED` として先に弾かれる。本関数に独立した容量チェックは不要である（`generateInitialRaces` 側の `LANE_CAPACITY_EXCEEDED` は別の話）。

なお現行実装はこれらを付録Aの `ProgressionError` ではなく、コード名を先頭に付けた `Error` として投げる（エンジン全体が `ProgressionError` を未実装であるため。付録Aとの乖離は未解消）。

この規則は満員時にはテンプレートの `bn` と完全に一致するため、通常のレースには影響しない。

適用例（6レーン、A版 7〜12艇パターンの FB = `[1:9.HT, 2:7.HT, 3:5.HT, 4:6.HT, 5:8.HT, 6:10.HT]`）:

| 生存艇 | 配置 |
|---|---|
| 6艇（満員） | テンプレートどおり `9.HT`=1, `7.HT`=2, `5.HT`=3, `6.HT`=4, `8.HT`=5, `10.HT`=6 |
| 4艇（`5〜8.HT`） | `5.HT`=3, `6.HT`=4, `7.HT`=2, `8.HT`=5 → レーン 2/3/4/5 |
| 3艇（`5〜7.HT`） | `5.HT`=3, `6.HT`=4, `7.HT`=2 → レーン 2/3/4 |
...
| EXC | 除外または棄権扱い。いずれにせよ非完漕であり進出対象外。DNS 相当か DQ 相当かは大会ルールによるが、進出判定はどちらでも変わらないため現行実装は §6.1 の許可リストにより一律 `unknown`（非完漕）として扱う。区別が必要になるのは監査ログの表示のみ |
...
`status` の正規化は**許可リスト**とする。比較前に前後の空白を除去し（値は CSV セル由来のため空白混入が日常的である）、明示的な `finish` だけを完漕として扱い、それ以外は**すべて非完漕**とする。

| 入力 | 正規化 | 進出 |
|---|---|---|
| `finish`（前後空白・大文字小文字は不問） | `finish` | 対象 |
| `DNS` / `DNF` / `DQ` | 同名 | 対象外 |
| `EXC`・未知のコード・綴り誤り | `unknown` | 対象外 |
| 空文字・空白のみ | `unknown` | 対象外 |
| status 欄そのものが無い（`undefined` / `null`） | `finish` | 対象（下記の互換規定） |

未知の値を `finish` と推測してはならない。推測すると完漕していない艇にレーンを与えることになり、誤りの方向が危険側になる。将来の競漕規則が新しいコードを追加した場合も、既定で非完漕に倒れる。

**空欄と「欄が無い」の区別**: この2つは別物として扱う。

- **status 欄そのものが無い**（`undefined` / `null`）= 旧 `race_NNN.json` は通常完漕の行で status を省略していたため、互換のため `finish` とする。
- **空文字・空白のみ** = 欄は存在するのに値が読めない、すなわち明示的に書かれた不明値である。付録Cは「空欄や `note` のみで状態が示される場合は正規化層で補完する」と規定しており、エンジンは正規化済みデータだけを受け取る建前なので、ここへ空欄が届くこと自体が**正規化層の漏れ**である。エンジンが `finish` と推測して埋めるとその漏れを隠してしまうため、`unknown` として進出対象から外す。

なお `undefined` を `finish` とする互換規定も、正規化層が全行を補完するようになれば不要になる。

**`RESULT_STATUS_INVALID` との関係（未解決）**: §4.4 は不明な status に対し `RESULT_STATUS_INVALID` を投げると規定するが、現行実装は例外を投げず `unknown` として黙って進出対象から外す。`strict` オプション有効時に例外へ切り替える案が自然だが、既存の呼び出し契約を変えるため未着手である。

### 6.2 欠員ランク（`__omitted`）

対象外艇によって空いたランク、およびエントリー数が満たず存在しないランクは、進出先のレーンを占有しない。エンジン内部ではこれを「占有者のいない既知のランク」として表現し、§5.2.1 の中央寄せ配置の入力に使う。

| 状態 | 意味 |
|---|---|
| ソースが存在し `__omitted` でない | 実在する艇。レーンを占有する |
| ソースが `__omitted` | ランクは定義されるが占有者なし（DNS/DNF/DQ で空いた枠） |
| ソースが存在しない | そのランク自体が存在しない（エントリー数不足） |

後者2つは §5.2.1 で**完全に同一**に扱う。どちらも「艇がいない」ことに変わりはなく、発艇表がその理由によって変わってはならないためである。`computeAdvancement` はどちらの場合も例外を投げず、その席を詰めて中央寄せし直す。

したがって `__omitted` は `computeAdvancement` の出力には影響しない（存在しないランクと同じ結果になる）。現状これが意味を持つのは、ランクが「定義されているが空」であることを呼び出し側・テストが区別したい場合だけである。単体で `assignLanes` を呼ぶ場合は `__omitted` の艇をレーンに着席させない。

> ⚠️ 未解決: `assignLanes` は §5.2.1 の中央寄せを行わずテンプレートの `bn` をそのまま使うため、同じ短編成でも `computeAdvancement` と異なる発艇表を返す。両者を揃えるか、`assignLanes` を内部用途に限定するかは未決。
